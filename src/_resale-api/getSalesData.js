  ...
  try {
    ...
    if (queryParams.propertyType == "commercial") {
      url = commercial.properties.replace(
        "$query",
        `?$filter=ContractStatus eq 'Available' and ${selectQuery}${rangeQuery}&$skip=${skipQuery}&$top=${limitQuery}&$orderby=OriginalEntryTimestamp desc`
      );
    } else {
      url = residential.properties.replace(
        "$query",
        `?$filter=ContractStatus eq 'Available' and StandardStatus eq 'Active' and ${selectQuery} ${rangeQuery}&$skip=${skipQuery}&$top=${limitQuery}&$orderby=OriginalEntryTimestamp desc`
      );
    }
    const options = {
      method: "GET",
