  ...
  try {
    ...
    if (queryParams.propertyType == "commercial") {
      ...
      url = residential.properties.replace(
        "$query",
        `?$filter=ContractStatus eq 'Available' and StandardStatus eq 'Active' and ${selectQuery} ${rangeQuery}&$skip=${skipQuery}&$top=${limitQuery}&$orderby=ModificationTimestamp desc,ListingKey desc`
      );
    }
